...
namespace WIMISODriverInjector.Core
{
    public class ImageProcessor
    {
        ...
        public ImageProcessor(Logger logger, Action<string>? guiLogCallback = null, Action<string>? statusUpdateCallback = null, Action<string>? phaseUpdateCallback = null, string? scratchDirectory = null)
        {
            _logger = logger;
            _guiLogCallback = guiLogCallback;
            _statusUpdateCallback = statusUpdateCallback;
            _phaseUpdateCallback = phaseUpdateCallback;
        ...
        {
            _phaseUpdateCallback?.Invoke(phase);
        }
        ...
        public async Task ProcessWIM(string wimPath, string outputWimPath, string[] driverDirectories, bool optimize, CancellationToken cancellationToken = default, List<int>? selectedIndexes = null)
        {
            ...
            try
            {
                ...
                for (int imgIdx = 0; imgIdx < indexesToProcess.Count; imgIdx++)
                {
                    ...
                    UpdatePhase($"[{imgIdx + 1}/{indexesToProcess.Count}] {index.Name}");
                ...
                else
                {
                    ...
                    // Phase already set to Finalizing... above
